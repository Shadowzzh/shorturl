...
func setDefaults() {
	...
	// 本服务只用 SQLite（见 database/init.go）。
	// 历史上这里的默认值曾指向 Postgres，而 database.driver 实际不被读取，
	// 结果是在没有 config.yaml 的目录里启动时，会把 Postgres 连接串当成 SQLite
	// 文件路径，直接 panic（unable to open database file）。现改为 SQLite 默认值。
